...
fn deposit_client(c: UnixEnd<(), Client>) {
...
fn withdraw_client(c: UnixEnd<(), Client>) {
...
fn main(){
    let client:UnixEnd<(), Client> = st_unix_client();
    let cli_t = thread::spawn(move || deposit_client(client));
    let _ = cli_t.join();

    // let client:UnixClient<(), Client> = st_unix_client();
    // let cli_t = thread::spawn(move || withdraw_client(client));
    // let _ = cli_t.join();
}